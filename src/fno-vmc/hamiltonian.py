...
def make_hamiltonian(ham_type: str, params: dict):
    ...
    hilbert = nk.hilbert.Spin(s=0.5, graph=graph)

    t = ham_type.lower()
    if t == "ising":
        J = params.get("J", 1.0)
        h = params.get("h", 1.0)
        op = nk.operator.Ising(hilbert, h=h, J=J)
    elif t == "heisenberg":
        J = params.get("J", 1.0)
        op = nk.operator.Heisenberg(hilbert, J=J)
    elif t == "xxz":
        Jx = params.get("Jx", 1.0)
        Jz = params.get("Jz", 1.0)
        op = nk.operator.XXZ(hilbert, Jx=Jx, Jz=Jz)
    else:
